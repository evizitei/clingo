...
bool event_handler(clingo_solve_event_type_t type, void *event, void *data, bool *goon) {
    ...
    switch (type) {
        case clingo_solve_event_type_statistics: {
            ...
            if (!clingo_statistics_root(stats, &root)) {
                return false;
            }

            // set some pseudo-random values in an array
            if (!clingo_statistics_map_add_subkey(stats, root, "values", clingo_statistics_type_array, &values)) {
                return false;
            }
            for (uint16_t i = 0; i < n; ++i) {
                random = (random << 3) ^ (random ^ ((random & 0xf800) >> 13)) ^ i;
                if (!clingo_statistics_array_push(stats, values, clingo_statistics_type_value, &value)) {
                    return false;
                }
                if (!clingo_statistics_value_set(stats, value, random)) {
                    return false;
                }
                sum += random;
            }
            ...
            if (!clingo_statistics_map_add_subkey(stats, root, "summary", clingo_statistics_type_map, &summary)) {
                return false;
            }
            if (!clingo_statistics_map_add_subkey(stats, summary, "sum", clingo_statistics_type_value, &value)) {
                return false;
            }
            if (!clingo_statistics_value_set(stats, value, sum)) {
                return false;
            }
            if (!clingo_statistics_map_add_subkey(stats, summary, "avg", clingo_statistics_type_value, &value)) {
                return false;
            }
            if (!clingo_statistics_value_set(stats, value, (double)sum / n)) {
                return false;
            }
            break;
        }
...
bool solve(clingo_control_t *ctl, clingo_solve_result_bitset_t *result) {
    ...
    // get a solve handle
    if (!clingo_control_solve(ctl, clingo_solve_mode_yield, NULL, 0, event_handler, NULL, &handle)) {
        goto error;
    }
...
int main(int argc, char const **argv) {
    ...
    if (!clingo_control_new(argv + 1, argc - 1, NULL, NULL, 20, &ctl)) {
        goto error;
    }
    ...
    if (!clingo_configuration_root(conf, &conf_root)) {
        goto error;
    }
    // and set the statistics level to one to get more statistics
    if (!clingo_configuration_map_at(conf, conf_root, "stats", &conf_sub)) {
        goto error;
    }
    if (!clingo_configuration_value_set(conf, conf_sub, "1")) {
        goto error;
    }
    ...
    if (!clingo_control_statistics(ctl, &stats)) {
        goto error;
    }
    if (!clingo_statistics_root(stats, &stats_key)) {
        goto error;
    }
    if (!print_statistics(stats, stats_key, 0)) {
        goto error;
    }
